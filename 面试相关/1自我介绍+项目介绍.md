...
- [项目介绍](#项目介绍)
  ...
  - [二、独立站 体验馆](#二独立站-体验馆)
    - [1、声音克隆](#1声音克隆)
    - [2、高光 2025.1-2025.2 （雪碧图获取 --\> 高光片段取差集获取蒙层片段）](#2高光-20251-20252-雪碧图获取----高光片段取差集获取蒙层片段)
    - [一、VideoLight高光时刻组件](#一videolight高光时刻组件)
    - [3、播放器](#3播放器)
    - [4、擦除](#4擦除)
  - [三、ai项目 -- specskills](#三ai项目----specskills)
- [SpecSkills](#specskills)
  - [🎯 什么是 SpecSkills](#-什么是-specskills)
    - [核心特性](#核心特性)
  - [🚀 快速开始](#-快速开始)
    - [前置要求](#前置要求)
    - [安装](#安装)
    - [初始化项目](#初始化项目)
    - [配置 MCP (前置条件)](#配置-mcp-前置条件)
      - [1. 从 MCP 市场安装](#1-从-mcp-市场安装)
      - [2. 手动配置 Chrome DevTools](#2-手动配置-chrome-devtools)
      - [3. 完整 MCP 配置示例](#3-完整-mcp-配置示例)
  - [🔄 工作流程](#-工作流程)
  - [📖 命令说明](#-命令说明)
    - [`/specskills:analyze`](#specskillsanalyze)
    - [`/specskills:proposal`](#specskillsproposal)
    - [`/specskills:apply`](#specskillsapply)
    - [`/specskills:review`](#specskillsreview)
    - [`/specskills:update-spec`](#specskillsupdate-spec)
  - [四、 运营系统 （比较简单）](#四-运营系统-比较简单)
  - [五、枝桠 （没涉及过）](#五枝桠-没涉及过)
...
### 2、高光 2025.1-2025.2 （雪碧图获取 --> 高光片段取差集获取蒙层片段）
### 一、VideoLight高光时刻组件
...
00:00.000 --> 00:10.000
demo_imageSprite_10_0.jpg#xywh=0,0,142,80

00:10.000 --> 00:20.000
demo_imageSprite_10_0.jpg#xywh=142,0,142,80

00:20.000 --> 00:30.000
demo_imageSprite_10_0.jpg#xywh=284,0,142,80

00:30.000 --> 00:40.000
demo_imageSprite_10_0.jpg#xywh=426,0,142,80
```
**3. 帧同步和动画控制**
...
帧间隔选择算法
高光图可以缩小放大，根据比例，选择根据多大的间隔展示对应的雪碧图
```tsx
...
const getDifferenceIntervals = (
  ...
  let result = [{ startTime: totalStart, endTime: totalEnd }];
  intervals.forEach((interval) => {
    const newResult: { startTime: any; endTime: any }[] = [];
    result.forEach((current) => {
      if (interval.endTime <= current.startTime || interval.startTime >= current.endTime) {
        newResult.push(current);
      } else {
        if (interval.startTime > current.startTime) {
          newResult.push({ startTime: current.startTime, endTime: interval.startTime });
        }
        if (interval.endTime < current.endTime) {
          newResult.push({ startTime: interval.endTime, endTime: current.endTime });
        }
      }
    });
    result = newResult;
...
[![npm version](https://img.shields.io/npm/v/specskills.svg)](https://www.npmjs.com/package/specskills)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

*通过项目感知的 AI，实现从需求到实现的结构化工作流*

[快速开始](#-快速开始) • [工作流程](#-工作流程) • [命令说明](#-命令说明)
...
**SpecSkills** 是一个 AI 编码助手的工作流增强工具，通过 **需求分析 → 方案归档 → 代码实现 → 质量评审** 实现结构化开发流程。

### 核心特性

- **📋 项目规范**: 使用项目特定的标准和约定指导 AI
- **🔄 结构化工作流**: 从分析到实现的分步命令
- **🎯 多阶段验证**: 在分析、实现和评审阶段确保质量
- **📚 知识积累**: 归档需求和模式供未来参考
...
- **Node.js** ≥ 22.0.0
- **AI 编码助手**: codebuddy IDE/CLI

### 安装

```bash
npm install -g @tencent/specskills
```
...
```
.codebuddy/
├── rules/specskills/      # 📋 项目规范和约定
├── skills/                # 🛠️ AI 能力模块
├── commands/specskills/   # ⚡ 工作流命令
└── agents/                # 🤖 子agent
...
MCP (Model Context Protocol) 可以增强 AI 的能力，提供更多集成功能。

#### 1. 从 MCP 市场安装

访问 [Knot MCP 市场](https://knot.woa.com/mcp/market) 搜索并安装以下插件:

- **TAPD** - 集成 TAPD 需求管理系统
- **Figma** - 集成 Figma 设计稿
- **Tea** - 集成 Tea 组件库文档

#### 2. 手动配置 Chrome DevTools

编辑 MCP 配置文件 (通常位于 `~/.codebuddy/mcp.json` 或项目的 `.codebuddy/mcp.json`):
...
```json
{
  "mcpServers": {
    "tapd_mcp_http": {
      ...
      "timeout": 20000,
      "headers": {
        "X-Tapd-Access-Token": "xxxxx"
      },
      "transportType": "streamable-http"
    },
    "tea-component-mcp-server": {
      "command": "npx",
      "args": [
        "-y",
        "@tencent/tea-component-mcp-server@latest"
      ],
      "env": {}
    },
    "chrome-devtools": {
...
用户输入需求
        ↓
┌──────────────────────┐
│  /specskills:analyze  │  澄清需求 → 识别变更 → 显示变更清单
└──────────┬───────────┘
           │
           ├─→ /specskills:summary (输出需求文档)
           │
           ├─→ /specskills:proposal (归档方案)
           ↓
┌──────────────────────┐
│  /specskills:apply   │  生成代码 → 多轮优化
└──────────┬───────────┘
           │
           ├─→ 简单 ──→ 完成
           │
           ├─→ 需评审 ──→ /specskills:review (质量评审)
           ↓
┌──────────────────────┐
│ /specskills:update-spec │  更新项目知识库
└──────────────────────┘
```
...
```bash
# 从需求描述
/specskills:analyze 添加带角色分配的用户管理功能

# 从 TAPD 链接
/specskills:analyze https://tapd.woa.com/xxx/story/1234567890
```

**处理流程**:
1. 读取项目规范 (`.codebuddy/rules/specskills/`)
2. 通过问答澄清需求
3. 搜索代码库定位受影响的文件
4. 在对话中显示变更清单
...
**前置条件**: 至少执行过一次 `/specskills:analyze`
...
**处理流程**:
1. 分析新增的组件/模式
2. 更新 `.codebuddy/rules/specskills/` (新规约)
